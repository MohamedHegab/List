class Ability
  ...
  def initialize(user)
    ...
    if user.admin?
      ...
      can [:update, :destroy], Comment, commentable_type: 'Card', commentable_id: Card.where(list_id: user.owns_lists.pluck(:id)).pluck(:id)
      can [:update, :destroy], Comment, commentable_type: 'Comment', commentable_id: Comment.where(commentable_type: 'Card', commentable_id: Card.where(list_id: user.lists.pluck(:id)).pluck(:id)).pluck(:id)
    else
